...
import com.lightningkite.mirror.ktor.HttpClientMultiRequestHandler
...
class MultiTest {
    ...
    val setupTestApplication: Application.() -> Unit = {
        ...
        routing {
            ...
            exposeMulti(handler, "request", false)
        }
    }
    ...
    val requestHandler = HttpClientMultiRequestHandler(
            client = HttpClient { },
    ...
    fun pingingMulti(){
        runBlocking {
            (0 .. 100).map {
                ...
                async { println(requestHandler.invoke(it)) }
            }.forEach {
                it.await()
            }
            println("Hits: ${requestHandler.hits.value}")
            assert(requestHandler.hits.value <= 2)
        }
    }
}